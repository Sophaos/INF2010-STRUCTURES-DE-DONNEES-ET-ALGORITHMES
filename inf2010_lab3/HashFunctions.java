...
*/
   ...
   public static ArrayList<Integer> randomIntegers(int length)
   {
      ...
      int size = 0;
      while(size < length){
         boolean randomTrouvé = false;
         int random = generator.nextInt(p);
         for(int i=0; i< size; i++){
            if(random == array.get(i)){
               randomTrouvé = true;
               i = size;
            }
         }
      ...
      /*
   La valeur de p a ete choisie de facon  ce qu' il soit très grand et premier.En effet, le remplissage du tableau se fait bien
      avec un facteur de compression beaucoup  moins que 50%.
     Oui, notre code reflete cette limite lorsqu'on appelle la fonction getKey(), la valeur donnée serait toujours modulo p
